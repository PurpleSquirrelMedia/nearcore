...
mod test {
    ...
    fn ban_store() {
        ...
        {
            let store = StoreBuilder::with_default_config().path(tmp_dir.path()).open();
        ...
        {
            let store_new = StoreBuilder::with_default_config().path(tmp_dir.path()).open();
            let peer_store_new =
    ...
    fn assert_peers_in_store(store_path: &std::path::Path, expected: &[PeerId]) {
        let store = StoreBuilder::with_default_config().path(store_path).open();
