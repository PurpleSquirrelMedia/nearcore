...
use near_store::{DBCol, Store, StoreBuilder};
...
fn apply_store_migrations(path: &Path, near_config: &NearConfig) -> anyhow::Result<()> {
    ...
    if cfg!(feature = "nightly") || cfg!(feature = "nightly_protocol") {
        let store = StoreBuilder::new(&near_config.config.store).path(path).open();
...
fn init_and_migrate_store(home_dir: &Path, near_config: &NearConfig) -> anyhow::Result<Store> {
    ...
    let store = StoreBuilder::new(&near_config.config.store).open();
    if !store_exists {
...
pub fn recompress_storage(home_dir: &Path, opts: RecompressOpts) -> anyhow::Result<()> {
    ...
    let src_store = StoreBuilder::new(&config.store).read_only(true).path(&src_dir).open();
    ...
    let dst_store = StoreBuilder::new(&config.store).path(&opts.dest_dir).open();
